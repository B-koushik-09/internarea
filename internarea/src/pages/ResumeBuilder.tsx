...
export default function ResumeBuilder() {
    ...
    const fetchMyResumes = async () => {
        if (user?._id) {
            try {
                const res = await axios.get(`http://localhost:5000/api/resume/my/${user._id}`);
    ...
    const createPayPalOrder = async (): Promise<string> => {
        try {
            const response = await axios.post("http://localhost:5000/api/resume/create-order", {
    ...
    const capturePayPalOrder = async (orderID: string) => {
        try {
            ...
            const captureRes = await axios.post("http://localhost:5000/api/resume/capture-order", {
                orderID,
                userId: user._id,
                details
            });

  ...
  useEffect(() => {
    if (user?._id) {
      ...
      const fetchData = async () => {
        try {
          const fRes = await axios.get(`http://localhost:5000/api/friend-routes/list/${user._id}`);
          const pRes = await axios.get(`http://localhost:5000/api/post-routes/my/${user._id}`);
          setStats({

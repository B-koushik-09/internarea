  ...
  useEffect(() => {
    if (user?._id) {
      ...
      const fetchData = async () => {
        try {
          const fRes = await axios.get(`https://internarea-wy7x.vercel.app/api/friend-routes/list/${user._id}`);
          const pRes = await axios.get(`https://internarea-wy7x.vercel.app/api/post-routes/my/${user._id}`);
          setStats({

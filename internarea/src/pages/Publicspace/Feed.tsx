...
export default function Feed() {
  ...
  const fetchPosts = () => {
    if (user?._id) {
      axios.get(`http://localhost:5000/api/post-routes/feed/${user._id}`)
  ...
  const like = async (id: string) => {
    try {
      await axios.post("http://localhost:5000/api/post-routes/like", { postId: id, userId: user?._id });
  ...
  const deletePost = async (id: string) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/post-routes/delete/${id}`);
  ...
  const submitComment = async (postId: string) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/post-routes/comment", {
        postId,
        userId: user._id,
        text: commentText
      });

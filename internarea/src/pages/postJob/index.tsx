...
const index = () => {
  ...
  const handlesubmit = async (e: React.FormEvent) => {
    ...
    try {
      setisloading(true);
      const res = await axios.post("https://internarea-production.up.railway.app/api/job", formData);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900">{t?.post_job_title || "Post New Job"}</h1>
            <p className="mt-2 text-sm text-gray-600">
              {t?.post_job_desc || "Create a new job opportunity"}
            </p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  <div className="flex items-center mb-1">
                    <Briefcase className="h-4 w-4 mr-1" />
                    {t?.post_label_about_job || "About Job"}*
                  </div>
                </label>
                <textarea
                  name="aboutJob"
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="text-black mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                  placeholder={t?.post_ph_ctc || "e.g. ₹10 LPA"}
                />
              </div>
              ...
              >
                {isloading ? (
                  <div className="flex items-center">
                    ...
                    {t?.post_posting_job || "Posting job..."}
                  </div>
                ) : (
                  t?.post_btn_job || "Post Job"
                )}
              </button>
            </div>
          </form>
